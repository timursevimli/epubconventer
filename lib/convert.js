...
const pathJoin = (targetPath, cli) =>
  path.join(cli ? process.cwd() : __dirname, targetPath);
const ensureTrailingSlash = (target) => {
  let res = target;
  if (!target.endsWith(path.sep)) res += path.sep;
  return res;
};
// prettier-ignore
...
class EpubConverter {
  ...
  constructor({ target = '.' + path.sep, output = target, cli = false } = {}) {
    this.#targetPath = generatePathStr(target, cli);
    this.#outputPath = generatePathStr(output, cli);
  }
